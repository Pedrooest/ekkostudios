...
import {
    ...
    Send, Link as LinkIcon, Filter, Copy, MoreHorizontal
} from 'lucide-react';
...
export default function AprovacaoTab() {
    ...
    const [posts, setPosts] = useState([
        {
            ...
            network: 'Instagram',
            format: 'Carrossel',
            status: 'pendente', // pendente, aprovado, alteracao
    ...
    const handleSendFeedback = () => {
        ...
        const newFeedback = {
            text: feedbackText,
            author: 'Equipe Interna', // Num cenário real, seria o nome do user logado ou do cliente
            time: 'Agora'
        };
    ...
    const handleCopyClientLink = () => {
        tryPlaySound('tap');
        // Simula copiar para o clipboard (navegador bloqueia clipboard nativo em iframes por vezes)
    ...
    return (
        <div className="flex h-[calc(100vh-80px)] overflow-hidden bg-[#F8FAFC] dark:bg-[#0a0a0c] font-sans transition-colors">
          ...
          ========================================= */}
                ...
                <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-3">
                    {filteredPosts.length === 0 ? (
                        ...
                        filteredPosts.map(post => (
                            <div
                                key={post.id}
                                onClick={() => { tryPlaySound('tap'); setSelectedPostId(post.id); setIsRejecting(false); }}
                                className={`p-4 rounded-2xl cursor-pointer transition-all border ios-btn flex gap-4 ${selectedPostId === post.id
                            ...
                            >
                                ...
                                <div className="flex-1 overflow-hidden">
                                    ...
                                    <h4 className={`text-sm font-bold truncate mb-1.5 ${selectedPostId === post.id ? 'text-indigo-900 dark:text-indigo-400' : 'text-gray-900 dark:text-zinc-200'}`}>
                                        {post.title}
                                    </h4>
            ...
            {/* =========================================
          COLUNA DIREITA: VISUALIZADOR DE POST (O "Pode Postar")
          ========================================= */}
            <div className="flex-1 flex flex-col relative bg-[#F8FAFC] dark:bg-[#0a0a0c]">
                {selectedPost ? (
                    <>
                        ...
                        <div className="px-6 sm:px-10 py-4 bg-white/80 dark:bg-[#111114]/80 backdrop-blur-md border-b border-gray-200 dark:border-zinc-800 flex justify-between items-center z-10 shrink-0">
                            <div>
                                <h3 className="text-sm font-bold text-gray-500 dark:text-zinc-400 uppercase tracking-widest flex items-center gap-2">
                                    <LayoutGrid size={16} className="text-indigo-500" /> Simulador de Aprovação
                                </h3>
                            </div>
                            <button
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 sm:p-10 flex justify-center">
                            ...
                            <div className="w-full max-w-4xl flex flex-col lg:flex-row gap-8 items-start">
                                ...
                                <div className="w-full lg:w-[400px] shrink-0 bg-white dark:bg-[#151518] rounded-[2rem] shadow-xl border border-gray-200 dark:border-zinc-800 overflow-hidden flex flex-col">
                                    ...
                                    <div className="p-4 border-b border-gray-100 dark:border-zinc-800/80 flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-yellow-400 via-rose-500 to-purple-600 p-[2px]">
                                            <div className="w-full h-full bg-white dark:bg-zinc-900 rounded-full flex items-center justify-center text-[10px] font-black text-gray-800 dark:text-zinc-200">
                                                {selectedPost.client.substring(0, 2).toUpperCase()}
                                            </div>
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-xs font-bold text-gray-900 dark:text-white">{selectedPost.client}</span>
                                    ...
                                    {/* Imagem/Vídeo */}
                                    <div className="w-full aspect-square bg-gray-100 dark:bg-zinc-900 relative">
                                        <img src={selectedPost.image} alt="Preview" className="w-full h-full object-cover" />
                                        {selectedPost.format === 'Reels' && (
                                    ...
                                    {/* Legenda (Copy) */}
                                    <div className="p-5 flex-1 bg-white dark:bg-[#151518]">
                                ...
                                {/* 2. PAINEL DE CONTROLE / APROVAÇÃO (Lado Direito) */}
                                <div className="flex-1 w-full max-w-[400px] flex flex-col gap-6">
                                    ...
                                    <div className="bg-white dark:bg-[#111114] p-6 rounded-2xl border border-gray-200 dark:border-zinc-800 shadow-sm">
                                        <h4 className="text-xs font-bold text-gray-500 dark:text-zinc-400 uppercase tracking-widest mb-4">Ação do Cliente</h4>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-gray-400 dark:text-zinc-600">
                        ...
                        <p className="text-sm font-medium mt-1">Selecione um conteúdo na lista lateral para aprovar.</p>
                    </div>
                )}
            </div>
